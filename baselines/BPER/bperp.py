...
class BPERp(nn.Module):
    ...
    def forward(self, user, item, exp, text, mask, train_flag=True, exp_emb_cache=None):
        ...
        if train_flag:
            ...
            assert exp_emb_cache is not None, "exp_emb_cache must be provided during evaluation"

            all_exp_num = exp.size(0)
            total_steps = int(math.ceil(all_exp_num / self.batch_size))
            ...
            for step in range(total_steps):
                start = step * self.batch_size
                end = min(start + self.batch_size, all_exp_num)
                
                exp_batch = exp[start:end]
                t_emb = exp_emb_cache[start:end].to(user_emb.device)  # (exp_batch_size, dimension)
                
                exp_u_emb = self.exp_u_embeddings(exp_batch)  # (exp_batch_size, dimension)
                exp_i_emb = self.exp_i_embeddings(exp_batch)
                exp_u_b = self.exp_u_bias(exp_batch)  # (exp_batch_size, 1)
                exp_i_b = self.exp_i_bias(exp_batch)
                
                eu_emb = (exp_u_emb * t_emb).t()  # (dimension, exp_batch_size)
                ei_emb = (exp_i_emb * t_emb).t()
                eu_b = exp_u_b.t()  # (1, exp_batch_size)
                ei_b = exp_i_b.t()
                
                u_score_batch = user_emb.matmul(eu_emb) + eu_b  # (batch_size, exp_batch_size)
                i_score_batch = item_emb.matmul(ei_emb) + ei_b
                
                u_scores.append(u_score_batch.detach().cpu()) 
                i_scores.append(i_score_batch.detach().cpu())
                
                del exp_u_emb, exp_i_emb, exp_u_b, exp_i_b, t_emb, eu_emb, ei_emb, eu_b, ei_b
                del u_score_batch, i_score_batch
                torch.cuda.empty_cache()
            
            u_score = torch.cat(u_scores, dim=1)  # (batch_size, all_exp)
            i_score = torch.cat(i_scores, dim=1)
            
            del u_scores, i_scores
            torch.cuda.empty_cache()
            
            return u_score, i_score
        ...
        """
            ...
            u_score = user_emb.matmul(eu_emb) + eu_b  # (batch_size, all_exp)
            i_score = item_emb.matmul(ei_emb) + ei_b
            #score = u_score * mu + i_score * (1 - mu)
            #topk = torch.topk(score, top_k, 1)[1]  # (batch_size, topk)
            return u_score, i_score  # (batch_size, all_exp)
        """
