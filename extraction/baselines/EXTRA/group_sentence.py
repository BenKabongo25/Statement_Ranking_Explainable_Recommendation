...
directory = dataset_dir + "tmp/EXTRA/"  # directory to save the grouped sentence ids
...
for idx, sentence in tqdm(enumerate(sentences), total=len(sentences), desc='Creating MinHash'):
...
for sim_threshold in sim_thresholds:  # create MinHash for once, when testing multiple similarity values
    ...
    exp_id_groups = []
    for idx, mh in tqdm(minhash_dict.items(), total=len(minhash_dict), desc='Grouping sentences'):
